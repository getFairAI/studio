...
const ModelCard = ({ tx, stampsCount }: { tx: IEdge; stampsCount: StampsCounts }) => {
    ...
    (async () => {
      if (txid && currentAddress) {
        ...
        const totalBalance = Object.keys(balances).reduce(
          (acc, curr) => acc + Number(balances[curr]),
          0,
        );
  ...
  return (
    <Card sx={{ display: 'flex', padding: '16px', alignItems: 'center', height: '100%' }}>
      <CardImage txid={txid} />
      <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
        <CardHeader
          title={findTag(tx, 'modelName')}
          subheader={
            'Last Updated on ' +
            new Date(parseInt(findTag(tx, 'unixTime') as string, RADIX) * 1000).toDateString()
          }
          sx={{ paddingTop: 0 }}
        />
        <CardContent>
          <Box display={'flex'} gap={'8px'}>
            <Typography sx={{ fontWeight: 500 }}>Total Inferences:</Typography>
            <Typography>{nInferences}</Typography>
          </Box>
          <Box display={'flex'} gap={'8px'}>
            <Typography sx={{ fontWeight: 500 }}>Owned Percentage:</Typography>
            <Typography>{ownedPercentage.toFixed(2)}%</Typography>
          </Box>
          <Box display={'flex'} gap={'8px'}>
            <Typography sx={{ fontWeight: 500 }}>Stamps:</Typography>
        ...
        >
          <Button
            ...
            className={`plausible-event-name=Edit+Model+Info plausible-event-modelId=${txid}`}
          >
            <Typography>Edit Information</Typography>
          </Button>
          <Button
            variant={'outlined'}
            onClick={handleViewInBazar}
            className={`plausible-event-name=Trade+Model plausible-event-modelId=${txid}`}
          >
...
const Models = () => {
  ...
  return (
    <Grid container spacing={2} padding={'16px'}>
      {models.map((tx: IEdge) => (
        <Grid item key={findTag(tx, 'modelTransaction')} xs={12} md={6} xl={4}>

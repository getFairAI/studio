...
const TermsAgreement = ({ children }: { children: ReactElement }) => {
  const { currentAddress, dispatchTx, disconnectWallet } = useContext(WalletContext);
  ...
  const { data } = useQuery(QUERY_TX_WITH, {
    variables: {
      ...
      tags: [
        ...
        { name: TAG_NAMES.operationName, values: [TERMS_STUDIO_AGREEMENT] },
  ...
  const handleAgreeClick = useCallback(async () => {
    ...
    const tx = await arweave.createTransaction({ data: TERMS_STUDIO_AGREEMENT });
  ...
  const handleBackClick = useCallback(() => disconnectWallet(), [disconnectWallet]);
